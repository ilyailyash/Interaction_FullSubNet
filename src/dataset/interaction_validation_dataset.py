...
class Dataset(BaseDataset):
    ...
    def __getitem__(self, item):
        ...
        speech_parent_dir = Path(noisy_file_path).parents[2].name
        ...
        if speech_parent_dir in ("dns_2_emotion", "dns_2_singing"):
            ...
            noise_file_basename = f"noise_fileid_{file_id}"

        clean_file_path = noisy_file_path.replace(f"noisy/{noisy_file_basename}", f"clean/{clean_file_basename}")
        noise_file_path = noisy_file_path.replace(f"noisy/{noisy_file_basename}", f"noise/{noise_file_basename}")

        noisy, _ = torchaudio.load(os.path.abspath(os.path.expanduser(noisy_file_path)))
        clean, _ = torchaudio.load(os.path.abspath(os.path.expanduser(clean_file_path)))
        noise, _ = torchaudio.load(os.path.abspath(os.path.expanduser(noise_file_path)))

        return noisy[0], clean[0], noise[0], reverb_remark + noisy_file_basename, speech_type
