...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for noisy, clean, noise in tqdm(self.train_dataloader, desc=f"Training {self.rank}"):
            ...
            with autocast(enabled=self.use_amp):
                ...
                enhanced = self.model(noisy_mag, noisy, noisy_complex, self.istft)
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (noisy, clean, noise, name, speech_type) in tqdm(enumerate(self.valid_dataloader), desc="Validation"):
            ...
            noisy_mag = noisy_mag.unsqueeze(1)
            enhanced = self.model(noisy_mag, noisy, noisy_complex, self.istft)
            complex_enhanced = self.torch_stft(enhanced)
            enhanced_mag, _ = mag_phase(complex_enhanced)
            loss = self.loss_function(enhanced_mag ** 0.3, clean_mag ** 0.3)
