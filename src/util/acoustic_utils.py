...
def stft(y, n_fft, hop_length, win_length, device):
    ...
    return torch.stft(y, n_fft, hop_length, win_length, window=window)
...
def load_wav(file, sr=16000):
    if len(file) == 2:
        ...
        try:
            return torchaudio.load(os.path.abspath(os.path.expanduser(file)))[0][0].numpy()
        except Exception as e:
            path = os.path.abspath(os.path.expanduser(file))
            #if os.path.exists(path):
            #    os.remove(path)
            print('cant load file ' + path)
            return torchaudio.load('/home/administrator/Data/DNS-Challenge'
                                   '/datasets/clean/read_speech/book_00000_chp_0009_reader_06709_0.wav')[0][0].numpy()
...
def overlap_cat(chunk_list, dim=-1):
    """
    Splicing chunk_list along the last dimension according to 50% overlap
