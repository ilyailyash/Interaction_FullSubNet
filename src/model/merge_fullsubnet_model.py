...
from src.model.module.merge import Merge
from src.model.interaction_fullsubnet_model import Model as Interaction
from src.util.acoustic_utils import drop_sub_band
from src.util.interactions import Interaction
from src.util.masks import get_wav_signal
from src.model.module.merge import Merge
...
class Model(BaseModel):
    def __init__(self,
        ...
        self.merge = Merge(frame_size=512, unfold_stride=256)
    ...
    def forward(self, input, noisy, *mask_args):
        """
        Args:
            input: [B, 1, F, T]
        ...
        if batch_size > 1:
            ...
            sband_input = sband_input.permute(0, 2, 1, 3)
        ...
        output_speech, output_noise = output_speech.permute(0, 2, 3, 1), output_noise.permute(0, 2, 3, 1)

        # merge branch
        speech_signal = get_wav_signal(output_speech, noisy.size(-1), *mask_args)
        noise_signal = get_wav_signal(output_noise, noisy.size(-1), *mask_args)
        enhanced = self.merge(noisy, speech_signal, noise_signal)
        return enhanced
